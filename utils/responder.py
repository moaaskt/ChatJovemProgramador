...
class Chatbot:
    def __init__(self):
        ...
        if not api_key:
            raise ValueError("API Key do Gemini não encontrada! Verifique seu arquivo .env")
        ...
        try:
            ...
            raise FileNotFoundError("Arquivo 'dados.json' não encontrado! Execute o scraper.py primeiro.")
    ...
    def _criar_contexto(self):
        ...
        if isinstance(noticias_formatadas, list) and noticias_formatadas:
             noticias_texto = "".join([f"• Título: {n.get('titulo', '')} (Link: {n.get('link', '')})\n" for n in noticias_formatadas])
        else:
